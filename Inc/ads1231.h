...
public:
  ...
  ADS1231(GPIO_TypeDef *powerdown_port, uint32_t powerdown_pin,
          SPI_HandleTypeDef *hspi, GPIO_TypeDef *miso_port, uint32_t miso_pin,
          GPIO_TypeDef *sclk_port, uint32_t sclk_pin, float m = 1, float b = 0);
...
private:
  static constexpr uint32_t kOffsetBinaryCodeZero =
      (1 << 23);  // 2^23 is the halfway point
