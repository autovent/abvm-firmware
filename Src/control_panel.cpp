...
ControlPanel::ControlPanel(
    // BUTTONS
    GPIO_TypeDef *sw_start_mode_port, uint16_t sw_start_mode_pin,
    GPIO_TypeDef *sw_stop_port, uint16_t sw_stop_pin,
    GPIO_TypeDef *sw_up_left_port, uint16_t sw_up_left_pin,
    GPIO_TypeDef *sw_dn_left_port, uint16_t sw_dn_left_pin,
    GPIO_TypeDef *sw_up_right_port, uint16_t sw_up_right_pin,
    GPIO_TypeDef *sw_dn_right_port, uint16_t sw_dn_right_pin,

    // STATUS LEDS
    GPIO_TypeDef *led_status1_port, uint16_t led_status1_pin,
    GPIO_TypeDef *led_status2_port, uint16_t led_status2_pin,
    GPIO_TypeDef *led_status3_port, uint16_t led_status3_pin,
    GPIO_TypeDef *led_status4_port, uint16_t led_status4_pin,

    // CHARLIEPLEXED BAR GRAPH LEDS
    GPIO_TypeDef *led_bar_left_char1_port, uint16_t led_bar_left_char1_pin,
    GPIO_TypeDef *led_bar_left_char2_port, uint16_t led_bar_left_char2_pin,
    GPIO_TypeDef *led_bar_left_char3_port, uint16_t led_bar_left_char3_pin,
    ...
    : buttons{{sw_start_mode_port, sw_start_mode_pin},
              {sw_stop_port, sw_stop_pin},
              {sw_up_left_port, sw_up_left_pin},
              {sw_up_right_port, sw_up_right_pin},
              {sw_dn_left_port, sw_dn_right_pin},
              {sw_dn_right_port, sw_dn_right_pin}},
...
void ControlPanel::set_buzzer_tone(BuzzerTone tone) {
  ...
  uint16_t oldCompare =
      __HAL_TIM_GET_COMPARE(buzzer_timer, buzzer_timer_channel);
  uint16_t newCompare =
      (uint16_t)((float)oldCompare * (float)newArr / (float)oldArr);
...
void ControlPanel::update() {
  if (HAL_GetTick() > last_charlie_update_ms + CHARLIE_UPDATE_INTERVAL_MS) {
    ...
    if (bar_states[BAR_GRAPH_LEFT].current > bar_states[BAR_GRAPH_LEFT].level) {
      bar_states[BAR_GRAPH_LEFT].current =
          bar_states[BAR_GRAPH_LEFT].level > 0 ? 1 : 0;
    }

    if (bar_states[BAR_GRAPH_RIGHT].current >
        bar_states[BAR_GRAPH_RIGHT].level) {
      bar_states[BAR_GRAPH_RIGHT].current =
          bar_states[BAR_GRAPH_RIGHT].level > 0 ? 1 : 0;
    }
  }
}
