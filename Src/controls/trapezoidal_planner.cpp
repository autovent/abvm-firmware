...
float TrapezoidalPlanner::run(float pos, float vel) {
  if (state == State::IDLE) {
    if (is_next_available) {
      ...
      t_counts_decel =
          (int)((params.t_d_percent * current.time_total_ms) / dt_ms);
      t_counts_c = (int)((current.time_total_ms *
                          (saturate(1 - params.t_a_percent - params.t_d_percent,
                                    0, 1))) /
                         dt_ms);

      current.time_total_ms =
