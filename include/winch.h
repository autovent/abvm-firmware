...
{
public:
  Winch(uint32_t p, Range<uint32_t> span_us = {600, 2400}, Range<float> span_degrees = {0, 180}, bool invert = false, float f = 50, uint32_t res = 16) : pin(p), span_us(span_us), span_degrees(span_degrees), freq(f), resolution(res), invert(invert)
  {
  }
